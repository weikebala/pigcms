 	 ...
 	 $this->data=D('Service_user');
 	  ...
 	  public function wechatService(){ 
 	  	if (IS_POST){ 
 	  		...
 	  		$this->success('设置成功');
 	  		 }else { $this->wxuser=S('wxuser_'.$this->token); 
 	  		 if (!$this->wxuser){ $this->wxuser=D('Wxuser')->where(array('token'=>$this->token))->find();
...
public function index(){ 
 	...
 	$count=$this->data->where($where)->count();
 	 $page=new Page($count,25);
 	  $list=$this->data->where($where)->limit($page->firstRow.','.$page->listRows)->order('id desc')->select(); $this->assign('page',$page->show()); 
 	  if ($list){ $sepTime=30*60; 
 	  	$nowTime=time(); $time=$nowTime-$sepTime;
 	  	 $i=0; foreach ($list as $item){ 
 	  	 	$list[$i]['online']=0; if ($item['endJoinDate']>$time){
 	  	 	 $list[$i]['online']=1; 
 	  	 	} $i++; 
 	  	 }
 	  	  } $this->assign('list',$list); 
 	  	  $this->assign('type','list'); $this->display(); 
 	  	} 
 	  	public function add(){
 	  	 if(IS_POST){ $db=D("Service_user"); 
 	  	   ...
 	  	   if($id==true){
 	  		...
 	  		$this->error('操作失败');
 	  		   ...
 	  		   public function edit(){
 	  		    if(IS_POST){ if(empty($_POST['userPwd'])){
 	  		       unset($_POST['userPwd']); 
 	  		   ...
 	  		   $this->all_save('Service_user','/index');
 	  		    	...
 	  		    	$where['id']=$this->_get('id','intval');
 	  		    	 ...
 	  		    	 $this->assign('serviceUser',$info); $this->display('add');
 	  		    	  }
 	  		    	   }
 	  		    	      ...
 	  		    	      $where['token']=session('token');
 	  		    	       $count=$data->where($where)->count();
 	  		    	        ...
 	  		    	        $list=$data->where($where)->limit($page->firstRow.','.$page->listRows)->order('id desc')->select();
 	  		    	         foreach($list as $key=>$vo){ $list[$key]['name']=D('Service_user')->getServiceUser($vo['pid']); 
 	  		    	     } 
