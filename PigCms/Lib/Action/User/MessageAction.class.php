...
class MessageAction extends UserAction{
    ...
    public function index3(){
        $this->assign('templateid','lZHxE1OB-ZS-NLB418Knfr3U3krlPxEXt_j237y1by0');
        $this->display();
    }
     ...
     * 消息模板发送
     */
    public function index3Ajax(){
        $post = $this->_post();
        if(empty($post['first'])){
            return $this->error('first参数参数不能为空');
        }
        if(empty($post['keyword1'])){
            return $this->error('keyword1参数参数不能为空');
        }
        if(empty($post['keyword2'])){
            return $this->error('keyword2参数参数不能为空');
        }

        if(empty($post['remark'])){
            return $this->error('remark参数参数不能为空');
        }
        if(empty($post['link'])){
            return $this->error('链接参数参数不能为空');
        }
        if(empty($post['templateid'])){
            return $this->error('templateid参数参数不能为空');
        }
        $row=array();
        $row['msgtype']= 'template';
        $param = array(
            'first' => $post['first'],
        ...
        $row['text']=serialize($param);
        $row['token']=$this->token;
        $row['time']=time();
        $id=M('Send_message')->add($row);
        //exec('D:\phpStudy\php\php-5.4.45\php.exe '.ROOT_PATH.'/cli/cliComment.php '.$id);
        $this->success('消息已创建，现在开始发送信息,粉丝过多需要一定时间');
    }
