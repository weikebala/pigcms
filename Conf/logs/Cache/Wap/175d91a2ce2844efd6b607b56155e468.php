		...
		<div id="container" class="container" <?php if($bgurl != NULL): ?>style="background:url('<?php echo ($bgurl); ?>') repeat-y center 0"<?php endif; ?>>
			<header>
				...
				<div class="header_2">
					<nav>
						<ul class="box">
							<li>
								<a href="<?php echo U('Forum/myContent',array('wecha_id'=>$_GET['wecha_id'],'token'=>$_GET['token']));?>" class="on">
									<label>帖子</label>
									<span><?php echo count($list);?></span>
								</a>
							</li>
							<li>
								<a href="<?php echo U('Forum/myLike',array('wecha_id'=>$_GET['wecha_id'],'token'=>$_GET['token']));?>">
									<label>喜欢</label>
									<span><?php echo ($mylikenum); ?></span>
								</a>
							</li>
							<li>
								<a href="<?php echo U('Forum/myMessage',array('wecha_id'=>$_GET['wecha_id'],'token'=>$_GET['token']));?>">
									<label>消息</label>
			...
			<div class="body">
				<div class="list_article">
					<article>
						<a style="color:#5d5d5d" href="<?php echo U('Userinfo/index',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'],'redirect'=>'Forum/myContent|wecha_id:'.$_GET['wecha_id']));?>"><div style="margin:10px;text-align:center;">修改个人资料</div></a>
					</article>
				</div>
				...
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><article>
						<header>
							...
							<ul class="tbox">
								...
								<li>
									<a href="/index.php?g=Wap&m=Forum&a=myContentEdit&tid=<?php echo ($list["id"]); ?>&wecha_id=<?php echo ($_GET['wecha_id']); ?>&token=<?php echo ($_GET['token']); ?>" class="icons icons_edit">&nbsp;</a>
								</li>
			...
			<footer>
				<section class="nav_footer">
					<ul class="box">
						<li>
							<a href="/index.php?g=Wap&m=Forum&a=index&wecha_id=<?php echo ($_GET['wecha_id']); ?>&token=<?php echo ($_GET['token']); ?>">
						...
						<li>
							<a href="/index.php?g=Wap&m=Forum&a=myMessage&wecha_id=<?php echo ($_GET['wecha_id']); ?>&token=<?php echo ($_GET['token']); ?>" class="on">
