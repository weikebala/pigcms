...
<ul>
        ...
        <?php if(is_array($vo['sub'])): $i = 0; $__LIST__ = $vo['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$res): $mod = ($i % 2 );++$i;?><li><a href="<?php if($res['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$res['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($res["url"])); endif; ?>"><div><img src="<?php echo ($res["img"]); ?>"></div><span><?php echo ($res["name"]); ?></span></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </div>
</div>
<script>
